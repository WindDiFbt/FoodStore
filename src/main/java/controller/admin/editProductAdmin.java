...
public class editProductAdmin extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (image != null && image.getSize() > 0) {
            ...
            img_path = product.getImage();
        }

        dao.updateProduct(new product(Integer.parseInt(product_id), Integer.parseInt(category_id),
                name, Integer.parseInt(discount), img_path, ingredient, user_manual, more_info, Timestamp.valueOf(LocalDateTime.now()),
                Integer.parseInt(price), Integer.parseInt(weight)));
